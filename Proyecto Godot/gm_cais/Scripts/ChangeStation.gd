	...
	if is_colli:
		if area_2d.monitoring:
			if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
				if event.pressed:
					get_parent().move_to_path(point_map, id_station, 1)

#Llamado al cambio de escena
func change_scene():
	transition.close_and_change(scene)

#Se activa el efecto de brillo y se ajusta el color
...
# Este método activa y desactiva el área de colisión
